...
int calSamples = 300;
int sensitivity = 8; // Just initiative - adjust sensitivity later in code for orbit/pan
int magRange = 3;
int outRange = 127;      // Max allowed in HID report
float xyThreshold = 0.5; // Center threshold 0.4
...
void loop()
{
  ...
  if (abs(xCurrent) > xyThreshold || abs(yCurrent) > xyThreshold)
  {
    ...
    // switching between orbiting and panning in OnShape and similar by observing zThreshold(zAxis)
    if (abs(zCurrent) < zThreshold && !isOrbit) // Joystick pressed down
    {
      Mouse.press(MOUSE_MIDDLE);  // Simulate middle mouse button press for panning
    }
    else if (abs(zCurrent) >= zThreshold && !isOrbit) // Joystick released
    {
      delay(20); // prevent random right clicks and crash
      Mouse.press(MOUSE_RIGHT);  // Simulate right mouse button press for orbiting
      sensitivity = 16; // higher=less_travel Adjust the sensitivity value as desired
      inRange = magRange * sensitivity;
      isOrbit = true;
    }

    // pan or orbit by moving proportionally to the xy axis
  ...
  else
  {
    ...
    Mouse.release(MOUSE_MIDDLE);  // Release middle mouse button
    Keyboard.releaseAll();
    sensitivity = 8; // reset the sensitivity value for panning
    inRange = magRange * sensitivity;
    isOrbit = false;
  }
...
// Set view to normal of sketch in OnShape (pressing "n")
void goHome()
{
  Keyboard.write('n');
...
void fitToScreen()
{
  ...
  // for other CAD-softwar double "middle mouse click"
  // Mouse.press(MOUSE_MIDDLE);
  // Mouse.release(MOUSE_MIDDLE);
  // Mouse.press(MOUSE_MIDDLE);
  // Mouse.release(MOUSE_MIDDLE);

  Serial.println("fit view");
}
